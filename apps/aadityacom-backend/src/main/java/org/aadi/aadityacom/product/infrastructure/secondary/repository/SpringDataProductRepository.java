...
import org.aadi.aadityacom.product.infrastructure.secondary.entity.CategoryEntity;
import org.aadi.aadityacom.product.infrastructure.secondary.entity.PictureEntity;
import org.aadi.aadityacom.product.infrastructure.secondary.entity.ProductEntity;
...
public class SpringDataProductRepository implements ProductRepository {
  ...
  public Page<Product> findByCategoryAndSize(Pageable pageable, FilterQuery filterQuery) {
    return jpaProductRepository.findByCategoryPublicIdAndSizesIn(
      pageable, filterQuery.categoryId().value(), filterQuery.sizes() //filterQuery.sizes(productSizes)
    ).map(ProductEntity::to);
  }
